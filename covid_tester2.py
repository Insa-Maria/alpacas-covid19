...
if __name__ == '__main__':
    ...
    st.sidebar.image(image_class,  width=50)
    ...
    infiltrate = st.sidebar.checkbox('Infiltrate')
    ...
    # Run button
    test_button = st.sidebar.button('Run Test')
    ...
    if(test_button):
        ...
        if(result[0][1]):
            ...
            symptom_array = np.array([cough,infiltrate,consolidation,shadows,fever,dyspnea,opacity])
            symptom_array= np.expand_dims(symptom_array, 0)
            p2 = model2.predict([image, symptom_array])
            print('P2: {}'.format(p2))
            p2 = int(round((p2[0][0])*10))
            print('P2: {}'.format(p2))
            model_severity = st.slider("Severity Case", 0, 10, p2)
